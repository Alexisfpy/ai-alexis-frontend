...
export default function Home() {
  ...
  // Estados para Google Workspace OAuth
  ...
  const verificarEstadoGoogle = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/google/status/${user.id}`);
      if (res.ok) {
        const data = await res.json();
      ...
      console.error('Error al comprobar estado de Google:', error);
    }
  };

  // Verificar estado de Google al iniciar sesión o volver del callback OAuth
  useEffect(() => {
    if (isSignedIn && user?.id) {
      ...
      if (params.get('google_auth') === 'success') {
        alert('✅ Cuenta de Google Workspace conectada correctamente.');
        verificarEstadoGoogle();
        window.history.replaceState({}, '', window.location.pathname);
      } else if (params.get('google_auth') === 'error') {
        const err = params.get('detail') || 'Error desconocido';
        alert(`⚠️ No se pudo conectar la cuenta de Google: ${err}`);
        window.history.replaceState({}, '', window.location.pathname);
      }
    }
  }, [isSignedIn, user]);
  ...
  const handleConectarGoogle = () => {
    if (!user?.id) return;
    // Redirige al endpoint de inicio de sesión de Google pasando el user_id
    window.location.href = `${API_URL}/google/login?user_id=${user.id}`;
  };

  const handleDesconectarGoogle = async () => {
    if (!confirm('¿Deseas desconectar tu cuenta de Google Workspace?')) return;
    setGoogleLoading(true);
    try {
      const res = await fetch(`${API_URL}/google/disconnect/${user.id}`, {
        method: 'POST'
      });
      if (res.ok) {
  ...
  // --- 8. ENVIAR MENSAJE CON STREAMING SSE ---
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden">
        ...
        <header className="flex items-center justify-between px-3 sm:px-6 py-3 bg-slate-900/80 border-b border-slate-800 backdrop-blur-md sticky top-0 z-10 w-full shrink-0 print:hidden">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* VINCULACIÓN GOOGLE WORKSPACE (OAUTH) */}
            {isLoaded && isSignedIn && (
              googleConnected ? (
                <div className="flex items-center gap-1.5 bg-slate-800/80 border border-emerald-500/40 px-2 sm:px-2.5 py-1 rounded-lg text-xs shadow-sm">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shrink-0" />
                  <span className="text-[11px] font-medium text-emerald-300 hidden md:inline truncate max-w-[130px]" title={googleEmail}>
                    {googleEmail}
                  </span>
                  <span className="text-[11px] font-medium text-emerald-300 md:hidden">
                    Google
                  </span>
                  <button
                    ...
                    className="text-slate-400 hover:text-red-400 ml-1 font-bold text-xs p-0.5 transition-colors"
                  >
                    ✕
                  </button>
                </div>
                ...
                <button
                  ...
                  disabled={googleLoading}
                  title="Conectar Google Calendar y Gmail"
                  className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-slate-600 px-2.5 sm:px-3 py-1.5 rounded-lg font-medium transition-all shadow-sm flex items-center gap-1.5 shrink-0"
                >
